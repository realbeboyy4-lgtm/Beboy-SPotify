...
<head>
    ...
    <title>Xiaomi Miner UI - Debug Mode</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #0f172a; color: #e2e8f0; }
        .json-box { font-family: 'Fira Code', monospace; }
...
<body class="min-h-screen p-4 md:p-8 flex items-center justify-center">
    <div class="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-4 space-y-4">
            ...
            <div class="bg-slate-800 border border-slate-700 rounded-2xl p-4 shadow-xl overflow-hidden">
                <div class="text-[10px] text-slate-400 font-bold uppercase mb-2">Last Server Payload</div>
                <div id="raw-response" class="bg-slate-900 p-3 rounded-lg text-[10px] text-blue-300 h-48 overflow-y-auto whitespace-pre-wrap json-box">{}</div>
            </div>
        </div>
        ...
        <div class="lg:col-span-8 flex flex-col gap-6">
            ...
            <div class="bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl flex flex-col overflow-hidden h-[600px]">
                ...
                <div id="log-window" class="p-6 text-xs font-mono overflow-y-auto space-y-1">
    ...
    <script>
        ...
        async function doLogin() {
            ...
            log(data.status === 'success' ? "✅ Session established!" : "❌ Error: " + data.msg);
        }

        async function sendCode() {
            const email = document.getElementById('email').value;
            await fetch('/send_code', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email})});
        ...
        function toggleMining() {
            ...
            if(isRunning) { log("🚀 Miner started...", "text-green-400"); scanLoop(); }
        }
        ...
        async function scanLoop() {
            ...
            try {
                ...
                document.getElementById('raw-response').innerText = JSON.stringify(d.data, null, 2);
                ...
                if(d.data && d.data.code === 0) {
                    ...
                    log(`🔸 [${attempts}] ${d.imei} : ${d.data.msg || d.data.code}`, "text-slate-500"); 
                }
            } catch (e) { log("⚠️ Net Error", "text-red-500"); }
            
            setTimeout(scanLoop, 1400);
        }
    </script>
</body>
</html>
